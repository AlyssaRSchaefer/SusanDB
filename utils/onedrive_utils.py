...
def get_student_folder_id(access_token, student_id):
    ...
    try:
        response = requests.get(check_url, headers=headers)
        if response.status_code != 200:
            logging.error(f"Error checking folder: {response.status_code} {response.text}")
            return None
        ...
        logging.error(f"Error getting student folder ID: {e}")
        return None
...
def create_student_folder(access_token, student_id):
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            logging.error(f"Error creating folder: {response.status_code} {response.text}")
            return None
    except Exception as e:
